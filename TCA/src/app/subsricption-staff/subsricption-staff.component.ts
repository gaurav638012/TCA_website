...
@Component({
  selector: 'app-subsricption-staff',
  templateUrl: './subsricption-staff.component.html',
  styleUrls: ['./subsricption-staff.component.css'],
  providers: [WindowRefService]
})
export class SubsricptionStaffComponent implements OnInit {
  ...
  //     return;
  //   }
  //   localStorage.setItem('interface_stage', "1");
  //   localStorage.setItem("ticket_type", "subscription");
  //   localStorage.setItem("contribution", this.f['contribution'].value);
  //   localStorage.setItem("additionalpasses", this.f['additionalpasses'].value);
  //   localStorage.setItem("slot", this.f['slot'].value);
  ...
  //      'email': localStorage.getItem('email'),'subscribed': 'yes', 'total_amount': this.total.toString(), 
  //      'contribution': this.f['contribution'].value,'extra_passes': this.f['additionalpasses'].value,
  //       'slot': this.f['slot'].value, 'booking_time': this.get_time(),
  //   }

  //   this.service.book_slot(this.f['slot'].value).subscribe(
  //     data =>{
  //      //console.log("step 1 booking s");
  //       this.Book(info,random_token);
  //       },
  //     error =>{
  //       this.clear_local();
  ...
  Book(random_token:any):any{
    ...
    var info = {
      'token':random_token,
      'name': localStorage.getItem('name'),'mobile': localStorage.getItem('mobile'), 'department': localStorage.getItem('department'),
       'email': localStorage.getItem('email'),'subscribed': 'yes', 'total_amount': this.total.toString(), 'contribution': this.f['contribution'].value,
      'extra_passes': this.f['additionalpasses'].value, 'slot': this.f['slot'].value, 'booking_time': this.get_time(),
    }

    this.service.book_ticket_staff(info)
      .subscribe(data => {
        this.service.book_slot(this.f['slot'].value).subscribe(data =>{          
           ...
           this.SendMail(localStorage.getItem('email'),document.location.origin+'/success_staff/'+url); 
           this.zone.run(() => {
            this.router.navigate(['/success_staff/'+url]);
        }
        );   
  ...
  Failure(){
    ...
    var url = this.generate_url(failure_tok);
    this.zone.run(() => {
      this.router.navigate(['/failure_staff/'+url]);
    });     
  }
